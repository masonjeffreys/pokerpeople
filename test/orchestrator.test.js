...
describe('handles standard side pot creation with full raise in same betting round',()=>{
    ...
    it('will allow a bet and create a side pot', () => {
        ...
        // BigBlind only has 75 chips (already has 10 in pot).
        // BigBlind goes all in with His first 30 call the first pot.
        // Other 45 go against side pot (60) but can't call it.
        // This would technicall potentially create a third side pot - 15 from small blind
        // This will never be called
        Orchestrator.receiveAction(game, 'all in');
        expect(Utils.potTotal(game.table.pots[0])).to.equal(160);
        expect(Utils.potTotal(game.table.pots[1])).to.equal(105);
